...
| `explosion_sampling_method` | `task.explosion_sampling_mode` + `src/utils.py::sample_explosion_point` | `without_replacement_cycle` (deterministic by block/condition seed) | `W2005240523` | Modified protocols describe bounded burst sampling per color. | `adapted` | Replaced global per-trial RNG draw for reproducibility and explicit policy. |
