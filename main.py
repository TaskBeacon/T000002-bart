...
def run(options: TaskRunOptions):
    """Run BART in human/qa/sim mode with one auditable flow."""
    task_root = Path(__file__).resolve().parent
    cfg = load_config(str(options.config_path))
    print(f"[BART] mode={options.mode} config={options.config_path}")
...
def main() -> None:
    ...
    options = parse_task_run_options(
        task_root=task_root,
        description="Run BART in human/qa/sim mode.",
